...
def fast_closest_pair(cluster_list, horizontal_order, vertical_order):
  '''
  ...
  if n <= 3:
    ...
    smallest_right = fast_closest_pair(cluster_list, horizontal_order_right, vertical_order_right)
    smallest = [smallest_left, smallest_right]
    smallest.sort()
    d = smallest[0]
    S = [v for v in vertical_order if abs(cluster_list[v].horiz_center() - mid) < d[0]]
    k = len(S)
    for u in range(k-1):
      upper = min(u+3,k-1)+1
...
def horiz_order(cluster_list):
  '''
  Takes a list of Cluster objects and return a list of indices generated by sorting the Cluster objects
  in nondecreasing order
  '''
  Horizontal_order = [(cluster_list[idx].horiz_center(), idx) for idx in range(len(cluster_list))]
  Horizontal_order.sort()
  Horizontal_order = [cluster[1] for cluster in Horizontal_order]
  return Horizontal_order

def vert_order(cluster_list):
  '''
  Takes a list of Cluster objects and return a list of indices generated by sorting the Cluster objects
  in nondecreasing order  
  '''
  Vertical_order = [(cluster_list[idx].vert_center(), idx) for idx in range(len(cluster_list))]
  Vertical_order.sort()
  Vertical_order = [cluster[1] for cluster in Vertical_order]
