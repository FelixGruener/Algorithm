...
factors <- as.vector(c(rep('computer_network',1348),
             rep('er p=0.001716433045422606',1348),
             rep('upa m =2',1348)))

df <- as.data.frame(cbind(number_of_nodes_removed,resilience,factors))

png("plot1.png", width = 800, height = 600)
ggplot(df, aes(x = number_of_nodes_removed, y = V1)) +
  geom_line(aes(group = factors, color= factors), size = 1, aplha = 0.5) +
